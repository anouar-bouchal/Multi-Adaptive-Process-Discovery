...
import pygad # SOTA Genetic Algorithm library
import time

# ============================================================================
# CONFIGURATION - SCALABILITY AUDIT (GENETIC ALGORITHM)
# ============================================================================
LOG_DIR = Path(__file__).parent / "prelogs"
REP_DIR = Path(__file__).parent / "reports"
SRC_LOG = LOG_DIR / "BPI Challenge 2017.pkl"
OUTPUT_FILENAME = "BPI17_Results_GENETIC_AUDIT.xlsx"
...
ALGO_NAME = "Genetic Algorithm (Evolutionary)"
SOLUTIONS_PER_POP = 10
NUM_GENERATIONS = 4 
# Budget: Initial Pop (10) + (10 * 4 Gens) = 50 evaluations
GA_EVAL_BUDGET = SOLUTIONS_PER_POP * (NUM_GENERATIONS + 1)
...
class SmartMiniCube:
    ...
    def process_strategy(self, strategy, iteration):
        """Processes the strategy using the Evolutionary/Genetic module."""
        if not self.activities: return
        
        evals_used = 0
        if self.is_triggered:
            # The Modular Swap: Replace Grid/Bayesian with Genetic
            best_p = self._run_genetic_evolution(strategy)
            evals_used = GA_EVAL_BUDGET
        ...
        self.history.append({
            ...
            # HARDWARE-INDEPENDENT MODULARITY OUTPUTS
            'opt_algo': ALGO_NAME,
            'eval_budget': evals_used,
            'efficiency_index': round(efficiency, 5)
        })

    def _run_genetic_evolution(self, strategy):
        """Standard GA implementation using PyGAD."""
        def fitness_func(ga_instance, solution, solution_idx):
            d, l, s = solution
            # Enforce bounds
            d, l, s = np.clip(d, 0.1, 0.9), np.clip(l, 0.4, 0.8), np.clip(s, 0.05, 0.25)
            m = self.adapted_HM(d, l, s)
            f, p, st = self.evaluate(m)
            
            # Primary target + secondary tie-breaker
        ...
        ga_instance = pygad.GA(num_generations=NUM_GENERATIONS,
                               ...
                               num_genes=3,
                               # Match the search space of Grid/Bayesian
        ...
        return {'dep': solution[0], 'loop': solution[1], 'sig': solution[2]}

    # --- Core Mining Logic (Kept identical for scientific control) ---
            ...
            if len(succ) > 1:
                ...
                for i in range(len(succ)):
                    for j in range(i+1, len(succ)):
                        b, c = succ[i], succ[j]
                        b_c, c_b = df.loc[b, c] if b in df.index and c in df.columns else 0, df.loc[c, b] if c in df.index and b in df.columns else 0
                        a_b, a_c = df.loc[act, b], df.loc[act, c]
                        qual += (1 - ((b_c + c_b)/(a_b + a_c + 1))); pairs += 1
...
class GeneticManager:
    ...
    def run(self):
        print(f"Running Evolutionary Modularity Audit on BPI 2017...")
        dim = DIMENSIONS[0]
        ...
        with pd.ExcelWriter(REP_DIR / OUTPUT_FILENAME) as writer:
            for group in all_res['group'].unique():
                all_res[all_res['group'] == group].sort_values(['strategy', 'iteration']).to_excel(writer, sheet_name=str(group)[:31], index=False)
...
if __name__ == "__main__":
    ...
    GeneticManager(event_log, COLUMNS, DIMENSIONS).run()
    print("Genetic Scalability Audit Complete.")
